...
| `payslips` | `{periodStart}_{staffId}` (corrections `_c{n}`) | Both | weekly payslip snapshots: staff/award/rate snapshot, hour buckets + rates, extra earnings, PAYG/deductions, super %, status (draft/under_review/approved/submitted/archived), audit[]. Staff read own **submitted/archived only** (rules deployed 2026-07-10) |
