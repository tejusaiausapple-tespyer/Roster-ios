...
/// no push entitlement needed, works today.
///
/// Reminder set per shift (all relative to the *rostered* start in the
/// business timezone):
...
enum ShiftReminderScheduler {
    private static let idPrefix = "shift-reminder."
    /// iOS caps pending local notifications at 64; 8 shifts × ≤7 reminders
    ...
    /// shifts change. `clockedInShiftId` suppresses the "forgot to start"
    /// nag for the shift already in progress and arms the end-of-shift one.
    static func sync(shifts: [Shift], clockedInShiftId: String?, now: Date = Date()) {
        ...
        center.getPendingNotificationRequests { pending in
            ...
            for shift in upcoming {
                for slot in slots {
                    ...
                    schedule(id: idPrefix + shift.id + "." + slot.tag,
                             title: slot.title, body: slot.body(shift),
                             fireDate: fireDate, shiftId: shift.id, now: now)
                }
                // End-of-shift reminder: only meaningful while clocked in.
                if shift.id == clockedInShiftId {
                    schedule(id: idPrefix + shift.id + ".forgot-end",
    ...
    private static func schedule(id: String, title: String, body: String,
                                 fireDate: Date, shiftId: String, now: Date) {
        guard fireDate > now else { return }
        let content = UNMutableNotificationContent()
        content.title = title
        content.body = body
        content.sound = .default
        content.userInfo = ["shiftId": shiftId, "kind": "shift-reminder"]
